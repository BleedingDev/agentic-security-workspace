...
def cmd_serve(args):
    ...
    if args.hf_model:
        # Serve different weights (e.g. an abliterated finetune) straight from Hugging Face:
        # skip the weights dataset so the kernel takes its download path.
        cfg["hf_model_id"] = args.hf_model
        cfg["weights_dataset"] = ""
        # e.g. huihui-ai/Huihui-Qwen3.8-27B-abliterated -> qwen3.8-27b-abliterated
        cfg["served_model_name"] = args.served_model_name or \
            re.sub(r"^huihui-", "", args.hf_model.split("/")[-1].lower())
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        (td / "kernel-metadata.json").write_text(json.dumps({
            ...
            "dataset_sources": ([] if args.hf_model else [args.weights_dataset]) + [ENV_DATASET],
            "competition_sources": [], "kernel_sources": [], "model_sources": [],
        }, indent=1))
        say(f"Pushing kernel {user}/{slug} (TPU v5e-8)..."
            + (f" serving {args.hf_model} from Hugging Face" if args.hf_model else ""))
    ...
    STATE_FILE.write_text(json.dumps(
        {"kernel": f"{user}/{slug}", "topic": topic, "api_key": api_key,
         "model": cfg.get("served_model_name", "qwen3.8-27b")}))
...
def main():
    ...
    s.add_argument("--hf-model", default=None, metavar="ORG/REPO",
                   help="serve these Hugging Face weights instead of the mirrored dataset "
                        "(same Qwen3.8-27B architecture, bf16 safetensors), e.g. "
                        "huihui-ai/Huihui-Qwen3.8-27B-abliterated; downloaded in-kernel (~5 min)")
    s.add_argument("--served-model-name", default=None,
                   help="model id the endpoint advertises (default: qwen3.8-27b, or derived "
                        "from --hf-model)")
